...
selected_nhom = None  # Biến global để lưu nhóm món ăn đã chọn

# Hàm để kết nối CSDL và truy vấn dữ liệu dựa trên nhóm món ăn
def lay_danh_sach_mon_an_theo_nhom():
    global selected_nhom  # Sử dụng biến global selected_nhom
    try:
        ...
        conn.close()
        message_label.config(text="Lấy danh sách món ăn thành công!")
    except Exception as e:
        message_label.config(text="Lỗi khi lấy danh sách món ăn: " + str(e))  # Hiển thị lỗi

# Hàm để chọn nhóm món ăn từ Combobox
def chon_nhom_mon_an(event):
    global selected_nhom  # Sử dụng biến global selected_nhom
    selected_nhom = nhom_mon_an_combobox.get()
    if selected_nhom != "Chọn nhóm món ăn":
        lay_danh_sach_mon_an_theo_nhom()
...
ket_noi_button = tk.Button(root, text="Kết nối và Hiển thị", command=lay_danh_sach_mon_an_theo_nhom)
...
label_nhom_mon_an = tk.Label(frame_nhom_mon_an, text="Nhóm món ăn:")
label_nhom_mon_an.pack(side=tk.LEFT, padx=5)  # Hiển thị label bên trái

# Tạo Combobox để chọn nhóm món ăn
nhom_mon_an_combobox = ttk.Combobox(frame_nhom_mon_an, values=["Chọn nhóm món ăn", "Hải sản", "Khai vị", "Bia- Nước ngọt", "Lẩu"])
